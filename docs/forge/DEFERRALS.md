...
## 2026-07-16 — Task 1 (Phase 7): one out-of-scope test still hard-codes the retired 2-iteration cap
**Why:** Task 1's brief scopes files to `scripts/forge_common.py` + `tests/test_forge_review.py` only; `MAX_ATTEMPTS_BACKSTOP = 5` (replacing `MAX_ATTEMPTS = 2`) is a mandatory rename shared by `forge-run.py`'s single call site (fixed here, unavoidable — otherwise the module fails to import and even the in-scope test file can't collect). The two cap-dependent tests inside `tests/test_forge_review.py` were updated to the new backstop value (5) as part of this task. One more, `tests/test_forge_dispatch.py::TimeoutTests::test_worker_timeout_counts_as_failed_iteration_then_escalates`, asserts escalation at `task-1-attempt-2.json` and is now red (`pytest tests -q`: 1 failed / 182 passed / 2 skipped) — same one-line mechanical cause, but that file is outside Task 1's declared scope so it was left as-is rather than editing an undeclared file.
**From:** Phase 7 Task 1 (verdict model + constants) execution.
**Follow-up:** fold into Task 2/3 (`scripts/forge-run.py` execute_task rewrite touches this same test file's fixtures) — bump `task-1-attempt-2.json` to `task-1-attempt-5.json` in that one test, same fix already applied to the two analogous cases in `tests/test_forge_review.py`.
