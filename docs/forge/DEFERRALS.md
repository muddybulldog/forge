...
## 2026-07-16 — Task 2 (Tier justification enforcement) breaks shared trivial/complex plan fixtures in other test files
**Why:** Task 2's scope is `scripts/forge_plan.py` + `tests/test_forge_plan.py` only. `parse_plan_tasks` now requires a non-empty justification for `trivial`/`complex` tiers, but `tests/_forge_support.py`'s shared fixtures (`PLAN_DEPS`, `PLAN_PASS`, `PLAN_TWO_TRIVIAL`, `PLAN_BAD_HEADING`, `PLAN_DUP`, etc.) use bare `**Tier:** trivial` with no justification. Those fixtures are consumed by `test_forge_dispatch.py`, `test_forge_loop.py`, `test_forge_receipts.py`, `test_forge_review.py`, `test_forge_status.py` — none named in Task 2's Files list. This task's own tests were kept green by defining locally-justified copies inline in `tests/test_forge_plan.py` rather than editing the shared fixture module, per Task 2's stated scope; the shared fixtures themselves were left untouched.
**From:** Task 2 execution (tier-policy-recalibration plan, 2026-07-16) — confirmed via full-suite run before/after (`python3 -m pytest tests/ -q`): 2 pre-existing failures unrelated to this change, 25 additional failures across the five files above once the justification enforcement lands.
**Follow-up:** add a mechanical justification suffix (e.g. `" — mechanical, single call site"`) to every bare `trivial`/`complex` `**Tier:**` line in `tests/_forge_support.py`'s shared fixtures; no other file needs it since `standard` (the floor) is exempt.
