...
**Proportional review:** trivial-tier tasks skip subagent review entirely; passing their acceptance commands is verification enough. Standard and complex tasks get the combined review — on the **dispatch** path the fresh-context subagent above; on the **inline** path the orchestrator's own self-review (no separate reviewer).
