...
1. **Explore context** — current files, docs, recent commits. Read `docs/theforge/DECISIONS.md` and `ROADMAP.md` if present; logged decisions are constraints — flag conflicts to the user, don't design around them silently. If the idea comes from `docs/theforge/ideas/` or a path handed at kickoff, and it's graduating to a build (not free-form ideation): read it, confirm your understanding, flag DECISIONS conflicts, skip questions it already answers, go straight to approaches.
